...
namespace wifibot
{
  ...
  bool Serial::read(BYTE *pBuffer, int size, int *pHasRead)
  {
    ...
    *pHasRead = ::read(_handle, pBuffer, size);
    ...
    if (*pHasRead < 0){
      std::cout <<"[libwifibot, Line 228] [Serial::read error]: Cannot read from serial"<<std::endl;
      return false;
    }
  ...
  {
	if(testdebug == true){
	//printf("\t\n **** I am in timeout SERIAL function LIBWIFIBOT \n");
	}
    fd_set rfds;
    struct timeval tv;
    int retval;
...
long odolefttmp = (p[8] << 24) + (p[7] << 16) + (p[6] << 8) + (p[5]);
    _data.speedFrontRight =  (p[10] << 8) + (p[9]);
    _data.adc[2] =            p[11];
    _data.adc[3] =            p[12];
    _data.odometryRight =    (p[16] << 24) + (p[15] << 16) + (p[14] << 8) + (p[13]);
long odorighttmp = (p[16] << 24) + (p[15] << 16) + (p[14] << 8) + (p[13]);
...
//if(testdebug ==true){
printf("\t ------- from LibWIFIbot Process ---------\n" );
printf("the Odo Right is  %0.3f\n", _data.odometryRight);
printf("the Odo Right [int] is  %d\n", odorighttmp);
printf("the Odo Left is  %0.3f\n", _data.odometryLeft);
printf("the Odo Left [int] is  %d\n", odolefttmp);
printf("the Speed Right is  %0.3f\n", _data.speedFrontRight);
printf("the Speed Left is  %0.3f\n", _data.speedFrontLeft);
printf("the _data.adc[0] is  %d\n", _data.adc[0]);
printf("the _data.adc[1] is  %d\n", _data.adc[1]);
printf("the _data.adc[2] is  %d\n", _data.adc[2]);
printf("the _data.adc[3] is  %d\n", _data.adc[3]);
...
//}
  ...
  {
	if(testdebug == true){
	//printf("\t I am into the GETData function LIBWIFIBOT \n");
	}
    return _data;
  }
  ...
  bool Protocol::setSpeeds(double left, double right)
  {
    ...
    //int16 sl = (5.0 * 50 * _ticsPerMeter) / 1000.0;
    //int16 sr = (5.0 * 50 * _ticsPerMeter) / 1000.0;
    BYTE c = 0;
  ...
  bool Frame::getFrame(BYTE *pFrame)
  {
	...
	if(testdebug == true){
	printf("\t\n **** I am into the getFrame function LIBWIFIBOT ----\n");
	}
	...
	if (_pBuffer[b] == 0xff)
	  {
		...
		//rcrc = ccrc; // this is just to remove crc check
// Yasir: i am going to quit the following linen as i need the data faster
	    if (rcrc == ccrc)
	      {
		if(testdebug == true){
...
if(testdebug == true){
  ...
  {
	int r;
	int countWhileLoopIterations =0;
 ...
 while (42)
	...
	/*if(testdebug == true)
	{
         countWhileLoopIterations++;
	if(_pFrame->getFrame(_protocol.getBufferOut()))
	printf(" *** Into Success While Loop in processRead function LIBWIFIBOT with success value:%d\n",countWhileLoopIterations);
}*/
  ...
  // Read the serial device, get a frame and fill the data structure
    ...
    if(testdebug == true){
printf("\t I have successfuly done Process Read in ReadData function in LIBWIFIBOT \n");
	}
    // Get the structure
